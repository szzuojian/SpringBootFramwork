 ...
 * Description: <br/>
 * Author: zuojian.xiang <br/>
 * Create Date: 2011-7-25 下午03:16:46 <br/>
 * Modify Info: <br/>
 * 
 * @version 1.0
 */
public class PropertyHelper {
	 ...
	 * @param value
	 */
	public static void setProperty(final Object bean, final String field, Object value) {
		setProperty(bean, field, value, false);
	}

	public static Object getProperty(final Object bean, final String field) {
		try {
	 ...
	 * @param value
	 */
	public static void setProperty(final Object bean, final String field, Object value, boolean ingoreNull) {
		try {
			// 如果对象是Map
			if (bean instanceof Map) {
				...
				PropertyDescriptor dis = PropertyUtils.getPropertyDescriptor(bean, field);
				...
				if (value == null) {
					...
					// 如果源是date
					if (nameType.equalsIgnoreCase("Date") || nameType.equalsIgnoreCase("Timestamp")) {
						// 目标也是date，则赋值，返回
						if (value instanceof Date) {
							...
							try {
								...
								if (tmp.indexOf("-") == -1) {
									...
									PropertyUtils.setProperty(bean, field, date);
								} else {
									Date date = sdf.parse(tmp.replaceAll("-", "").substring(0, 8));
									PropertyUtils.setProperty(bean, field, date);
								}
							} finally {
						...
						if (nameType.equalsIgnoreCase("BigDecimal")) {
							...
							if (!"Class".equals(nameType)) {
								throw new RuntimeException("不支持的类型" + nameType);
							}
						}
					}
				}
	...
	public static boolean compareObj(Object srcValue, Object changedValue) {
		if (srcValue == null) {
			...
			if (srcValue instanceof Date && changedValue instanceof Date) {
				...
				return ((Date) srcValue).getTime() == ((Date) changedValue).getTime();
			} else {
